        ...
        try {
            ...
            if (axios.isAxiosError(error) && error.response) {
                setError({
                    ...
                    url: error.response.data.url || "/administracion/valoraciones",
                    color: 2,
                    ...
                    <div className="scroll overflow-x-auto  max-h-[100%] w-full rounded-t-2xl scroll-smooth">
                        <table className="table-auto w-full bg-white border border-gray-200 rounded-tl-2xl">
                            ...
                            <tr className="min-h-[4rem]">
                                ...
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-200)">Pedido_id</th>
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-100)">Plato_id</th>
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-200)">Username</th>
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-100)">Valoracion</th>
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-200)">Descripcion</th>
                                <th className="px-6 py-4 border-b whitespace-nowrap bg-(--oxley-100)">Fecha</th>
                            ...
                            {datos.map(valoracion => (
                                <tr key={valoracion.id}>
                                    <td className="px-6 py-4 border-r border-b whitespace-nowrap text-center bg-(--oxley-100) font-bold">{valoracion.id}</td>
                                    <td className="px-6 py-4 border-b whitespace-nowrap text-center bg-(--oxley-200)">{valoracion.pedido_id}</td>
                                    <td className="px-6 py-4 border-r border-b whitespace-nowrap text-center bg-(--oxley-100) font-bold">{valoracion.plato_id}</td>
                                    <td className="px-6 py-4 border-b whitespace-nowrap text-center bg-(--oxley-200)">{valoracion.username}</td>
                                    <td className="px-6 py-4 border-r border-b whitespace-nowrap text-center bg-(--oxley-100) font-bold">{valoracion.valoracion}</td>
                                    <td className="px-6 py-4 border-b whitespace-nowrap text-center bg-(--oxley-200) truncate max-w-xs" title={valoracion.descripcion}>{valoracion.descripcion}</td>
                                    <td className="px-6 py-4 border-r border-b whitespace-nowrap text-center bg-(--oxley-100) font-bold">{valoracion.fecha}</td>
